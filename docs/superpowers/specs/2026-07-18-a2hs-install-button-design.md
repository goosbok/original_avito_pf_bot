...
Mobile-only: the whole matrix applies only on viewports ≤768px. On desktop widths (≥769px) both the banner and the header icon are hidden via CSS regardless of state — adding to a home screen is a mobile action, and the product does not want the prompt on desktop. Consistent with the stylesheet's single 768/769px cut and its `.mobile-only` convention (a landscape phone >768px is treated as desktop everywhere in this app).
...
- `InstallGuideSheet` — iOS instructions. Mounted once at app root; opened by CustomEvent `a2hs-open-guide` (same pattern as `support-chat-send`). Always a bottom sheet (its triggers only exist on mobile), no open/close animation — appears instantly. Three steps: «Поделиться» **в панели браузера** (wording covers both Safari and Chrome/Firefox on iOS) → «На экран “Домой”» → «Добавить». Ghost button «Готово, добавил» (→ `dismiss()` + close), closes on overlay click.
...
New block near other component styles: `.a2hs-banner`, `.a2hs-header-btn`, `.a2hs-sheet` (+ overlay). Theme via existing CSS vars only (`--surface`, `--border`, `--primary`, `--text-*`) — must look right in light and dark. Breakpoints (all on the stylesheet's single 768/769px cut): `@media (min-width: 769px)` hides both the banner and the header icon (mobile-only feature); `@media (max-width: 359px)` additionally hides the header icon on ultra-narrow phones (the cramped header row clips the burger), leaving the banner as the sole entry point there. The guide sheet is always a bottom sheet (it can only be opened from the mobile-visible triggers). Per project rule, verify on mobile AND desktop — on desktop the whole feature must be absent.
...
1. Chromium install flow on a MOBILE viewport (feature is hidden on desktop width): once `beforeinstallprompt` fires the banner + header icon appear; «Установить» opens the native dialog; **declining keeps both visible** (further clicks are no-ops until the event re-fires); accepting hides both immediately (`appinstalled`); relaunch in the installed window shows neither (standalone). Separately confirm that at desktop width (≥769px) neither the banner nor the icon renders even with `beforeinstallprompt` fired.
...
6. Both themes (light/dark) for banner, sheet AND header icon on mobile widths; confirm all three are absent at desktop width. Include the cramped 320–375px mobile header on an admin account (admin toggle + install icon + bell + theme toggle in one row).
