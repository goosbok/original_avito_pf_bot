...
- Уведомление админам в TG со ссылкой на вкладку и числом строк.
...
- `export_auto_launches() -> tuple[str, int]` — синхронная, вызывает
  `create_auto_tasks_sheet()`, возвращает `(url, rows)`.
