...
def _effective_start_msk(now_msk: datetime) -> date:
    ...
    Бизнес-сутка биза начинается в 04:00 МСК (следом идёт start_hour =
    PF_DEFAULT_START_HOUR). Заказы до 04:00 МСК **включительно**
    запускаются в эту же дату; заказы строго после 04:00 МСК переносятся
    на следующий календарный день. Все заказы интервала
    (T-1).04:01 .. T.04:00 МСК попадают в одну партию запуска —
    T в PF_DEFAULT_START_HOUR МСК.
...
def _build_avito_payload(
    ...
    # завтрашней даты в PF_DEFAULT_START_HOUR МСК. Иначе биза не успевает
    # за остаток дня
