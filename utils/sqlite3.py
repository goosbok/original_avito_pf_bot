...
def get_auto_launched_links(days: int = 30):
    ...
    Границы окна инклюзивны с обеих сторон (`>= date('now', '-N days')`
    захватывает и сегодня, и день `сегодня-N`), поэтому фактически
    попадает `days + 1` календарная дата, а не `days`. Это осознанно: для
    задачи важно не потерять свежие ссылки, а расширение окна на день не
    мешает сверке с дашбордом биза — «чинить» это не нужно.
    """
    with sqlite3.connect(path_db) as con:
        con.row_factory = dict_factory
        sql = (
